"""
Bank Marketing Classification — Decision Tree
==============================================
Improvements applied (v2):
  - One-hot encoding for nominal features (fixes ordinal assumption)
  - Ordinal encoding kept for education
  - BorderlineSMOTE for class imbalance
  - Wider GridSearchCV: deeper trees, more pruning params
  - Precision-recall threshold tuning for best F1
  - Results saved to results/DecisionTree/
"""
...
# ── Output folder ────────────────────────────
RESULTS_DIR = os.path.join("results", "DecisionTree")
os.makedirs(RESULTS_DIR, exist_ok=True)

# ── 1. Load Data ─────────────────────────────
print("=" * 55)
print("Decision Tree — Bank Marketing (v2)")
print("=" * 55)
...
# One-hot encoding for other categorical columns (avoids imposing ordinal relationships)
...
param_grid = {
    "clf__max_depth":              [6, 8, 10, 12, 15, 20],
...
metrics = {
    ...
    "F1-Score":        round(f1,   4),
    "ROC-AUC":         round(auc,  4),
    "Best max_depth":  search.best_params_["clf__max_depth"],
    "Best min_samples_leaf": search.best_params_["clf__min_samples_leaf"],
    "Best min_samples_split": search.best_params_["clf__min_samples_split"],
    "Best criterion":  search.best_params_["clf__criterion"],
    "Best max_leaf_nodes": search.best_params_.get("clf__max_leaf_nodes", None),
}
...
ax.plot(fpr, tpr, lw=2, color="#1f77b4", label=f"Decision Tree (AUC = {auc:.3f})")
...
ax.set_title("Precision / Recall / F1 vs Threshold — Decision Tree")
ax.legend(); ax.grid(alpha=0.3)
plt.tight_layout()
...
# Feature Importance
importances = pd.Series(best_pipeline.named_steps["clf"].feature_importances_, index=X.columns)
top15 = importances.nlargest(15).sort_values()
fig, ax = plt.subplots(figsize=(7, 6))
top15.plot(kind="barh", ax=ax, color="#1f77b4", edgecolor="black")
